...
os.environ['ALLOWED_API_KEYS'] = 'api-key-1,api-key-2'
...
def test_coordinates_success_with_default_province_and_country(client, mocker):
    mock = mocker.patch('src.googlemaps_geocoding_adapter.GoogleMapsGeocodingAdapter.get_location', 
                        return_value=LocationResult(Location(
                            ...
                            country="CA")))
    response = client.get(
        "/coordinates?address=1234+Main+St&city=Vancouver",
        headers={
            'api-key': 'api-key-1'
        })
    assert response.status_code == 200
...
def test_coordinates_success_with_custom_province_and_country(client, mocker):
    ...
    response = client.get(
        "/coordinates?address=1234+Main+St&city=Vancouver&province=CA&country=US",
        headers={
            'api-key': 'api-key-2'
        })
    assert response.status_code == 200
...
def test_coordinates_unauthorized(client):
    response = client.get(
        "/coordinates?address=1234+Main+St&city=Vancouver",
        headers = {
            'api-key': 'invalid-api'
...
def test_coordinates_missing_api_key(client):
    response = client.get(
        "/coordinates?address=1234+Main+St&city=Vancouver")
    assert response.status_code == 401
    assert response.json() == {"message": "Unauthorized"}

def test_coordinates_missing_address(client):
    response = client.get(
        "/coordinates?city=Vancouver",
        headers = {
            'api-key': 'api-key-1'
        })
    assert response.status_code == 422
    assert response.json() == {"detail": [{"input": None, "loc": ["query", "address"], "msg": "Field required", "type": "missing"}]}

def test_coordinates_missing_city(client):
    response = client.get(
        "/coordinates?address=1234+Main+St",
        headers = {
            'api-key': 'api-key-1'
        })
    assert response.status_code == 422
    assert response.json() == {"detail": [{"input": None, "loc": ["query", "city"], "msg": "Field required", "type": "missing"}]}
